...
Once the reverse proxy receives requests, it hands them over to the final receiver, a service. If there are replica sets of pods (which is not the case in this demo project), random ports are assigned accordingly, and the proxy will perform load balancing over the replicas.
